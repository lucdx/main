...
. `Listing all Fees`
//TODO implement list and tracking feature. Also may update addfeees command to be edit instead.

An Example of how command 1 - `Editing the Fee of a Person` works:

. The admin will be able to use the 'addfees' command to add a new fee to the Person.
. The existing fee with its date will be replaced by this new value in the Person object in the AddressBook.
. The updated fees will then be store in the AddressBook and will be automatically private, only viewable by the admin.
...
//Not for long
* **Alternative 1 (current choice):** Loop through the InternalList created and prints every Person's name and Fees
** Pros: Easy to implement.
** Cons: Does not allow for much data manipulation of the list such as sorting.
//TODO
* **Alternative 2:**
** Pros:
** Cons: 
